...
export const PUT = async (
    ...
    try {
        const client = await clientPromise;
        const db = client.db('remarker_next');
        const plan = await db.collection('plans').findOneAndUpdate(
            { _id: new ObjectId(id) },
            { $set: planData },
        ...
        return NextResponse.json({ message: "Plan successfully updated", data: plan }, { status: 200 });
...
export const DELETE = async (req: NextRequest, { params }: {
    ...
    try {
        ...
        return NextResponse.json({ message: "Plan successfully deleted", data: plan }, { status: 200 });
