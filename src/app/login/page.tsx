  ...
  return (
    <Suspense fallback={<LoginLoading />}>
      <div className="w-full h-screen flex flex-col items-center justify-start px-4 py-2 text-black gap-y-3">
        ...
        >
          <h1
            className={`text-xl font-medium mb-1 ${emailError || passwordError ? "text-red-500" : "text-[#0ea5e9]"}`}
          >
            Sign In
          </h1>
          <span className="text-md font-normal text-gray-500 mb-4">
            Please enter your {step === 1 ? "email" : "password"}
          </span>
        ...
        {step === 2 && (
          <button
            onClick={handleBack}
            className="flex items-center py-1 px-2 rounded-lg text-white bg-[#22d3ee]"
          >
