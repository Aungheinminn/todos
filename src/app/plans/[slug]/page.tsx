...
const PlanHeader: React.FC<PlanHeaderProps> = ({ plan, isPlanLoading }) => {
  if (isPlanLoading) {
  ...
  return (
    <div className="relative w-full flex justify-between items-center p-1 mb-4">
      ...
      <Link
        href="/plans"
        className="absolute top-0 right-2 border-2  bg-[#0ea5e9] rounded-md text-gray-800 flex justify-start items-center gap-x-1"
      >
        <Image src={back} alt="back" className="w-5 h-4 ml-1" />
        <p className="text-white font-normal mr-2 mb-[2px]">Back</p>
      </Link>
      <p className="absolute bottom-2 left-5 text-md text-[#0ea5e9] font-medium">
        {plan.name}
      </p>
    </div>
  );
    ...
    <div className="w-full grid grid-cols-2 px-1 gap-1 mb-4">
      ...
      <div className="flex justify-start items-center border-2 border-[#0ea5e9] h-[60px] rounded-lg">
        <div className="flex flex-col mx-2">
          ...
          <p className="text-sm text-[#C0C0C0] ml-8 line-clamp-1">
            Total Routines
          </p>
        </div>
      </div>
        ...
        <div className="cursor-pointer w-full flex flex-col gap-y-1">
          {routines &&
            routines.map((routine) => (
              <div
                key={routine._id}
                onClick={() => handleOpenDetailPopup(routine)}
                className="relative transition duration-300 ease-in-out group hover:border-[#C0C0C0] w-full h-[45px] flex items-center justify-start border-2 border-b-4 border-[#0ea5e9] rounded-md"
              >
                <p className="w-[120px] truncate transition duration-300 ease-in-out text-md font-medium text-[#0ea5e9] m-2 group-hover:opacity-0">
                  {routine.name}
                </p>
                <p className="absolute left-1/2 -translate-x-1/2 transition duration-300 ease-in-out text-md font-medium text-[#C0C0C0] m-2 opacity-0 group-hover:opacity-100">
                  {" "}
                  Click for details
                </p>
              </div>
...
const Plan = ({ params }: { params: { slug: string } }) => {
  const { slug } = params;
