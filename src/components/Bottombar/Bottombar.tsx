...
const Bottombar = () => {
    const [active, setActive] = useState<'home' | 'create' | 'settings'>('create')
    const activeInterface = " absolute z-[100] border-4 border-gray-800 -translate-y-9 bg-[#4ade80] rounded-full"
    const handleChange = (value: 'home' | 'create' | 'settings') => {
        setActive(value)
    }
    console.log(active)
    return (
        <div className="w-full flex justify-between items-center bg-gray-800 lg:hidden px-8 h-[50px]">
