...
const PopoverComponent = ({
  ...
  const { openPopup: openDeletePopup, popupData: deletePopupData } =
    useDeletePopupStore((state) => state);
  const { openPopup: openEditPopup, popupData: editPopupData } =
    useEditPopupStore((state) => state);
  const handleDelete = () => {
...
const CardComponent = ({
  ...
  const date = plan.createdAt
    ? new Date(plan.createdAt).toUTCString().split(" ").slice(0, 4).join(" ")
    : "";
    ...
    <div className="transition-all w-full flex flex-col justify-start gap-3 bg-[#cbd5e1] border-2 border-[#78717C] p-2 rounded-lg cursor-pointer hover:border-[#5b21b6]">
      ...
      <div className="flex justify-between items-center pb-2">
        <div className="flex justify-start items-center gap-x-2 text-black">
          <span className="text-md text-[#78717C] font-medium">
            Created At:
          </span>
